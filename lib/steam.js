 ...
 * @constructor
 */
let steam = function(obj) {
  let validFormats = ['json', 'xml', 'vdf'];
...
steam.prototype.getGlobalAchievementPercentagesForApp = function(input) {
  let obj = this.normalizeAppGameId(input);
...
steam.prototype.validateIPv4 = function(ip) {
  let ipformat = /^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/g;
...
steam.prototype.makeRequest = function(obj) {
  ...
  return new Promise((resolve, reject) => {
    http.get(options, (res) => {
      ...
      res.on('end', () => {
        if (statusCode === 404) {
          reject('404 Error was returned from steam API');
          return;
        } else if (statusCode === 403) {
          reject('403 Error: Check your API key is correct');
          return;
        }
        if (format === 'json') {
          try {
            ...
            reject('JSON response invalid, your API key is most likely wrong');
            return;
          }
        }
